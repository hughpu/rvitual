...
void check_gl::opengl_check_error(const char *filename, int lineno,
                                  const char *expr) {
  GLenum err = glad_glGetError();
  if (err != GL_NO_ERROR) {
    std::cerr << filename << ":" << lineno << ": " << expr
              << " failed: " << opengl_errno_name(err) << '\n';
    std::terminate();
  }
}
...
static void APIENTRY opengl_debug_message_callback(GLenum source, GLenum type,
  ...
  switch (source) {
#define PER_GL_DEBUG_SOURCE(x)                                                 \
  case GL_DEBUG_SOURCE_##x:                                                    \
    _source = #x;                                                              \
    break;
...
#undef PER_GL_DEBUG_SOURCE
  ...
  switch (type) {
#define PER_GL_DEBUG_TYPE(x)                                                   \
  case GL_DEBUG_TYPE_##x:                                                      \
    _type = #x;                                                                \
    break;
...
#undef PER_GL_DEBUG_TYPE
  ...
  switch (severity) {
#define PER_GL_DEBUG_SEVERITY(x)                                               \
  case GL_DEBUG_SEVERITY_##x:                                                  \
    _severity = #x;                                                            \
    break;
...
#undef PER_GL_DEBUG_SEVERITY
  ...
  fprintf(stderr,
          "OpenGL message [%d]: %s of %s severity, raised from %s: %s\n", id,
          _type, _severity, _source, msg);
  /* #ifndef NDEBUG */
  /* #if defined(_MSC_VER) */
  /*         __debugbreak(); */
...
void check_gl::opengl_try_enable_debug_message() {
  ...
  if (flags & GL_CONTEXT_FLAG_DEBUG_BIT) {
    if (opengl_has_extension("GL_ARB_debug_output")) {
      auto ptr_glDebugMessageCallbackARB =
          (PFNGLDEBUGMESSAGECALLBACKPROC)glfwGetProcAddress(
...
void check_gl::opengl_show_glfw_error_diagnose() {
  ...
  } else if (!strcmp(errmsg,
                     "WGL: The driver does not appear to support OpenGL")) {
    std::cerr << "** Please consider install an OpenGL driver, or use the mesa "
                 "driver\n";
  } else if (!strcmp(errmsg, "WGL: Failed to create OpenGL context")) {
    std::cerr
        << "** Your driver seems not supporting the required OpenGL version\n";
  }
  std::cerr << "- If you have a physical graphic card (e.g. NVIDIA), install "
               "it from your graphic card vendor official website: "
               "http://www.nvidia.com/Download/index.aspx\n";
  std::cerr << "- If you are using Windows, download opengl32.dll from "
               "https://pan.baidu.com/s/1TZ6nVJC7DZIuUarZrGJYow?pwd=opgl and "
               "place it into the same directory as this executable file "
               "(alternatively you may download opengl32sw.dll from Internet "
               "and rename it to opengl32.dll to place into the same directory "
               "as this executable file)\n";
  std::cerr
      << "- If you are using Linux server or WSL1, install the mesa driver: "
         "https://ubuntuhandbook.org/index.php/2021/07/"
         "install-latest-mesa-ubuntu-20-04-21-04/";
  std::cerr
      << "- If you use WSL2, install WSLg: "
         "https://learn.microsoft.com/zh-cn/windows/wsl/tutorials/gui-apps\n";
  std::cerr << "- If you are using SSH connection for remote server, try "
               "connect it using ssh -X <ip address>\n";
  std::cerr << "- If you are using MacOS, you probably want to use Windows or "
               "Linux instead for better OpenGL support\n";
  std::cerr << "- If you are using a Laptop with dual-cards, make sure you "
               "have switch to dedicated card (NVIDIA) instead of the "
               "integrated card (Intel)\n";
  std::cerr << "==============================================\n";
#ifdef _WIN32
  std::system("pause");
#endif
...
void check_gl::opengl_shader_source(unsigned int shader,
